...
describe('resolveOutcome', () => {
  it('resolves group-match outcomes, including voids', () => {
    ...
    st.results['gA-david-galim'] = { sets: [], voided: true };
    expect(R('m:gA-david-sanek:david', st)).toBe('won');
    expect(R('m:gA-david-sanek:sanek', st)).toBe('lost');
    expect(R('m:gA-david-galim:david', st)).toBe('void');
    expect(R('m:gA-david-misha:david', st)).toBe('pending');
  });

  it('resolves group-exit only once the group is complete', () => {
    const partial = emptyState();
    partial.results['gA-david-sanek'] = { sets: aWin2 };
    expect(R('exit:david', partial)).toBe('pending');

    const st = fullFavoriteState();
    expect(R('exit:david', st)).toBe('won'); // A1
    expect(R('exit:misha', st)).toBe('won'); // A2
  ...
  it('resolves champion, reach-final and the final-score specials', () => {
    const st = fullFavoriteState();
    expect(R('champ:david', st)).toBe('won');
    expect(R('champ:tolyan', st)).toBe('lost');
    expect(R('final:david', st)).toBe('won');
    expect(R('final:tolyan', st)).toBe('won');
    expect(R('final:islam', st)).toBe('lost');
    expect(R('sp:final-3-0', st)).toBe('won');
    expect(R('sp:final-3-1', st)).toBe('lost');
    expect(R('sp:final-david-tolyan', st)).toBe('won');
  });

  it('resolves specials that hinge on upsets and last place', () => {
    // no upsets when every favorite wins
    ...
    // a single big upset flips it immediately, before the tournament ends
    const upset = emptyState();
    upset.results['gA-david-sanek'] = { sets: [{ a: 2, b: 11 }, { a: 2, b: 11 }] };
    expect(R('sp:upset-3plus', upset)).toBe('won');
    expect(R('sp:vanek-or-sanek-wins', upset)).toBe('won');
  });

  it('blocks betting on your own defeat', () => {
    // "sanek beats david" -> david would be backing his own loss
    expect(isSelfLoss('david', OUTCOME_BY_ID['m:gA-david-sanek:sanek'])).toBe(true);
    expect(isSelfLoss('sanek', OUTCOME_BY_ID['m:gA-david-sanek:sanek'])).toBe(false);
    expect(isSelfLoss('misha', OUTCOME_BY_ID['m:gA-david-sanek:sanek'])).toBe(false);
    expect(isSelfLoss('david', OUTCOME_BY_ID['champ:tolyan'])).toBe(false);
  });
});
